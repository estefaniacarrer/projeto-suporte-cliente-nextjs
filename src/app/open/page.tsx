...
export default function OpenTicket(){
    ...
    const [customer, setCustomer ] = useState<CustomerDataInfo | null>(null) //para teste ({})
    ...
    return(
        <div className="w-full max-w-2xl mx-auto px-2">
            ...
            <main className="flex flex-col mt-4 mb-2">
               ...
               {customer ? (
                <div className="bg-sky-50 py-6 px-4 rounded flex items-center justify-between">
                    <p className="text-lg"><strong>Cliente selecionado:</strong> {customer.name}</p>
                    <button className="h-8 px-2 flex items-center justify-center rounded" onClick={handleClearCustomer}>
                        <FiX size={30} color="#ff2929" />
                    </button>
                </div>
               ):(
                <form 
                    className="bg-sky-50 py-6 px-2 rounded "
